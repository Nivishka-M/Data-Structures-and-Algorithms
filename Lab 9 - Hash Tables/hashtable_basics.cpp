...
struct HashTable{
	...
	//add your code below
        for(int i = 0; i< user_name.length(); i++){
            sum += int(user_name[i]);  //getting ascii sum for each character in the key string
	...
	//add an if condition to complete the code here
        if (empty){
            password[hash]=user_password;   //inserting the password if the corresponding slot is empty
        }
        else{
            cout << "The slot for the key is already occupied"<<endl;  
        }
	...
	//add an if condition to complete the code here
        if(!empty){
            cout << "The password is :"<< password[hash] << endl;    
        }
        else{
            cout << "There is No password for this key"<< endl;
        }
